...
ANCHOR_HOST="peer0.org2.${DOMAIN}"
ANCHOR_PORT="${ORG2_PEER_PORT}"
...
echo "==> Set Org2MSP anchor peers -> ${ANCHOR_HOST}:${ANCHOR_PORT}"
jq --arg host "$ANCHOR_HOST" --argjson port "$ANCHOR_PORT" \
  '(.channel_group.groups.Application.groups.Org2MSP.values.AnchorPeers.value.anchor_peers)=[{"host":$host,"port":$port}]' \
