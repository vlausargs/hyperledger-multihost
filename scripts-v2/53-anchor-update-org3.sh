...
ANCHOR_HOST="peer0.org3.${DOMAIN}"
ANCHOR_PORT="${ORG3_PEER_PORT}"
...
echo "==> Set Org3MSP anchor peers -> ${ANCHOR_HOST}:${ANCHOR_PORT}"
jq --arg host "$ANCHOR_HOST" --argjson port "$ANCHOR_PORT" \
  '(.channel_group.groups.Application.groups.Org3MSP.values.AnchorPeers.value.anchor_peers)=[{"host":$host,"port":$port}]' \
